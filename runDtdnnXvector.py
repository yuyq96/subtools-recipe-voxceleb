...
"""A launcher script with python version (Yu's launcher to do experiments w.r.t dtdnn-xvector.py).
...
model_params = {
    "mixup":False,
    "specaugment":False,
    "aug_dropout":0., "context_dropout":0, "hidden_dropout":0.,

    "xvector_params":{"init_dim":128,
                      "layers":[6, 12],
                      "growth_rate":64,
                      "bn_scale":2,
                      "nonlinearity":"relu",
    ...
    "margin_loss":True,
    "margin_loss_params":{
            "method":"aam", "m":0.25, "feature_normalize":True, 
            "s":32, "mhe_loss":False, "mhe_w":0.01},
...
optimizer_params = {
    ...
    "beta2":0.999,
    "beta3":0.999,
    "weight_decay":5e-4, # Should be large for decouped weight decay (adamW) and small for L2 regularization (sgd, adam).
    "lookahead.k":5,
...
model_blueprint = "subtools/recipe/voxceleb-dtdnn/dtdnn-xvector.py"
model_dir = "exp/dtdnn"
##--------------------------------------------------##
##
######################################################### START #########################################################
...
if stage <= 4 <= endstage and utils.is_main_training():
    ...
    to_extracted_data = ["voxceleb1", "voxceleb2_dev"] # All dataset should be in data_root/prefix.
    ...
    try:
        ...
        for epoch in to_extracted_epochs:
            ...
            point_name = "epoch_{0}".format(epoch)
